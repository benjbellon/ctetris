...
void test_step_matrix_down() {
  ...
  Tetromino_t *dummy = Tetromino_init(TETROMINO_SHAPE_T, 0, 0);

  TetrominoCollection_t *col = TetrominoCollection_init(1);
  TetrominoCollection_push(col, dummy);

  Tetromino_t *under_test[3][5] = {{NULL, NULL, dummy, dummy, dummy},
  ...
  for (size_t row = 0; row < board->rows; row++) {
    memcpy(board->arr[row], under_test[row],
           board->cols * sizeof(Tetromino_t *));
  }
...
void test_collision_check() {
  ...
  Tetromino_t *not_locked = Tetromino_init(TETROMINO_SHAPE_T, 0, 0);
  Tetromino_t *locked = Tetromino_init(TETROMINO_SHAPE_I, 0, 0);
  ...
  Tetromino_t *board_in[3][5] = {
      {locked, NULL, not_locked, not_locked, not_locked},
      {locked, NULL, NULL, not_locked, NULL},
      {locked, NULL, NULL, NULL, NULL}};

  Tetromino_t *prev_board_in[3][5] = {
      {NULL, NULL, not_locked, not_locked, not_locked},
  ...
  for (size_t row = 0; row < board->rows; row++) {
    memcpy(prev->arr[row], prev_board_in[row],
           board->cols * sizeof(Tetromino_t *));
  }

  GameBoard_collision_check(board, prev);

  TEST_ASSERT_EQUAL_INT_MESSAGE(TETROMINO_STATE_ACTIVE, not_locked->state,
                                "Expected not_locked to be active");
  TEST_ASSERT_EQUAL_INT_MESSAGE(TETROMINO_STATE_LOCKED, locked->state,
                                "Expected locked to be locked");
}
...
void test_collision_check_with_intersection() {
  ...
  Tetromino_t *not_locked = Tetromino_init(TETROMINO_SHAPE_T, 0, 0);
  Tetromino_t *locked = Tetromino_init(TETROMINO_SHAPE_I, 0, 0);
  Tetromino_t *another = Tetromino_init(TETROMINO_SHAPE_I, 0, 0);
  ...
  Tetromino_t *prev_board_in[3][5] = {
      {another, NULL, not_locked, not_locked, not_locked},
  ...
  TEST_ASSERT_EQUAL_INT_MESSAGE(TETROMINO_STATE_ACTIVE, not_locked->state,
                                "Expected not_locked to be active");
  TEST_ASSERT_EQUAL_INT_MESSAGE(TETROMINO_STATE_INTERSECTED, another->state,
                                "Expected another to be intersected");
  TEST_ASSERT_EQUAL_INT_MESSAGE(TETROMINO_STATE_INTERSECTED, locked->state,
                                "Expected locked to be intersected");
}

void test_tetromino_collection_contains_active() {
  TetrominoCollection_t *coll = TetrominoCollection_init(3);
  Tetromino_t *t1 = Tetromino_init(TETROMINO_SHAPE_I, 0, 0);
  Tetromino_t *t2 = Tetromino_init(TETROMINO_SHAPE_J, 0, 0);
  Tetromino_t *t3 = Tetromino_init(TETROMINO_SHAPE_L, 0, 0);
...
void test_rotate_right_pi_radians() {
  GameBoard_t *board = GameBoard_init(3, 5);
  Tetromino_t *tet = Tetromino_init(TETROMINO_SHAPE_I, 0, 0);

  TetrominoCollection_t *col = TetrominoCollection_init(3);
  TetrominoCollection_push(col, tet);

  Tetromino_t *initial[3][5] = {{NULL, NULL, tet, NULL, NULL},
                                {NULL, NULL, tet, NULL, NULL},
                                {NULL, NULL, tet, NULL, NULL}};

  Tetromino_t *expected[3][5] = {{NULL, NULL, NULL, NULL, NULL},
                                 {NULL, tet, tet, tet, NULL},
                                 {NULL, NULL, NULL, NULL, NULL}};
...
void test_get_tetromino_coords() {
  ...
  Tetromino_t *initial[3][5] = {{tet, tet, tet, tet, NULL},
                                {NULL, NULL, NULL, NULL, NULL},
                                {NULL, NULL, NULL, NULL, NULL}};
  ...
  for (size_t row = 0; row < actual->rows; row++) {
    memcpy(actual->arr[row], initial[row],
           actual->cols * sizeof(Tetromino_t *));
  }
...
void test_translate_right() {
  ...
  Tetromino_t *expected[3][5] = {{NULL, tet, tet, tet, tet},
                                 {NULL, NULL, NULL, NULL, NULL},
                                 {NULL, NULL, NULL, NULL, NULL}};
...
void test_translate_left() {
  ...
  Tetromino_t *initial[3][5] = {{NULL, tet, tet, tet, tet},
                                {NULL, NULL, NULL, NULL, NULL},
                                {NULL, NULL, NULL, NULL, NULL}};

  Tetromino_t *expected[3][5] = {{tet, tet, tet, tet, NULL},
                                 {NULL, NULL, NULL, NULL, NULL},
                                 {NULL, NULL, NULL, NULL, NULL}};
  ...
  GameBoard_print_debug(board);
  GameBoard_translate_left(board, tet);
  GameBoard_print_debug(board);
